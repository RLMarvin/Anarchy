...
class Anarchy(BaseAgent):
    ...
    def get_output(self, packet: GameTickPacket) -> SimpleControllerState:
        ...
        team_sign = (
            1
            if self.car.team
            == (is_1v1 and packet.teams[self.car.team].score == 7 and packet.teams[not self.car.team].score == 0)
            else -1
        )
        ...
        impacts = [
            get_impact(ball_prediction, car, self.time, self.renderer) for car in packet.game_cars[: packet.num_cars]
        ]
        ...
        for i in range(100):
            ...
            self.renderer.draw_line_3d(
                ...
                self.renderer.create_color(255, random.randint(0, 255), random.randint(0, 255), random.randint(0, 255)),
            )
        ...
        for index, car in enumerate(packet.game_cars[: packet.num_cars]):
            if car.team == self.team and index != self.index:
                ...
                teammate_correct_side_of_ball: bool = ((impacts[index][0].y - teammate_location.y) * team_sign > 0)
                ...
                if impacts[index][1] + 0.1 < impact_time or (
                    ...
                    self.renderer.draw_line_3d(car_location, teammate_location, self.renderer.black())
                    self.renderer.draw_rect_3d(teammate_location, 4, 4, True, self.renderer.black())
                    self.renderer.end_rendering()
                    break
        ...
        need_boost = not self.car.is_super_sonic and self.car.boost < (40 if convenient_boost else 30)
        park_car = False
        ...
        if kickoff and not not_our_kickoff:
            ...
            destination.x += max(car_to_ball.length / 3, 80 if wait else 120) * -sign(impact_projection.x)
        else:
            destination += (destination - enemy_goal).normalized * max(
                (destination - car_location).length / ((2.1 + impact_time / 1.9) if take_serious_shot else 3.7),
                60 if wait else 110,
            )
            take_serious_shot = True
        if abs(car_location.y) > 5120:
            destination.x = min(700, max(-700, destination.x))  # Don't get stuck in goal
        car_to_destination = destination - car_location

        # Rendering
        self.renderer.begin_rendering()
        if kickoff:
            self.renderer.draw_string_2d(20, 140, 2, 2, "Kickoff", self.renderer.white())
        if take_serious_shot:
            self.renderer.draw_string_2d(20, 140, 2, 2, "Shoot", self.renderer.white())
        self.renderer.draw_line_3d(
            [destination.x, destination.y, impact.z], [impact.x, impact.y, impact.z], self.renderer.blue()
        )
        if avoid_own_goal:
            self.renderer.draw_line_3d(
        ...
        local = rotation_matrix.dot(
            Vector3(
                ...
                (impact.z if wall_touch else 17.010000228881836) - self.car.physics.location.z,
            )
        )
        ...
        if slow_down:
            ...
            self.renderer.draw_string_2d(
                ...
                6,
                "Uh Oh",
                self.renderer.pink() if (self.time % 0.5) < 0.25 else self.renderer.red(),
            )
            self.renderer.end_rendering()
        turning_radians = self.steer_correction_radians
        backwards = math.cos(turning_radians) < 0 and not demoing and not self.car.is_super_sonic
        if backwards:
        ...
        if park_car:
            ...
            if car_velocity.length > 400:
                self.controller.throttle = -sign(rotation_matrix.data[0].dot(destination - car_location))
            ...
            if self.controller.boost and self.mode is Gamemode.DROPSHOT:
                self.controller.boost = 30 < self.car.boost - (2200 - car_velocity.length) / (
        ...
        if (
            ((car_to_ball.flatten().size < 400 and ball_location.z < 300) or dodge_for_speed)
